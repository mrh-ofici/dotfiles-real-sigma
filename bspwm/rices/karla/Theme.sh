...
set_bspwm_config() {
	...
	bspc config normal_border_color "#5884d4"
...
set_picom_config() {
 	sed -i "$HOME"/.config/bspwm/picom.conf \
		...
		-e "s/corner-radius = .*/corner-radius = 30/g" \
...
launch_bars() {
	...
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
		(MONITOR=$mon polybar -q karla-bar -c ${rice_dir}/config.ini)&		(MONITOR=$mon polybar -q karla-bar2 -c ${rice_dir}/config.ini)&
